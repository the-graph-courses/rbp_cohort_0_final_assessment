...
To access the source code for this report, click the link below to download the final assessment folder. Unzip the download, and click into the "rmd" subfolder. The "ng_example" document there is the file you seek.

[**DOWNLOAD FINAL ASSESSMENT FOLDER**](https://minhaskamal.github.io/DownGit/#/home?url=https://github.com/the-graph-courses/rbp_cohort_0_final_assessment){target="_blank"}

Note that the dataset used in that example is not included in the main assessment folder due to size limitations. This means that if you want to run the "ng_example" Rmd yourself, you will need to download the source data separately. You can find and download this dataset---the Nigerian 2018 DHS survey---[here](https://drive.google.com/drive/u/0/folders/1J0gw4qpfbB8bYFbXx8QcsQEWgJSVwp_P){target="_blank"}, in the same folder that houses the other compiled datasets.
